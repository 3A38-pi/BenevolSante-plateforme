...
final class ArticleController extends AbstractController
{
    ...
    //         // 2. Vérifier la validité via votre service IA (injection dans le constructeur)
    //         //    Exemple : $this->openAIService->isCommentAcceptable($commentContent)
    //         if (!$this->openAIService->isCommentAcceptable($commentContent)) {
    //             // 3. Gérer le cas où le commentaire est jugé inapproprié
    //             $this->addFlash('danger', 'Ce commentaire contient un contenu inapproprié.');
                
    //             // Rediriger pour afficher le message d’erreur (ou rester sur la même page)
    //             return $this->redirectToRoute('articleDescription', [
    //                 'id' => $article->getId(),
    //             ]);
    //         }
    
    //         // 4. Si le commentaire est acceptable, on l’enregistre normalement
    //         $commentaire->setArticle($article);
    //         $commentaire->setUser($this->getUser());
    //         $this->em->persist($commentaire);
    //         $this->em->flush();
    ...
    //     // Affichage de la page avec le formulaire et la liste des commentaires
    //     return $this->render('templates_users/articleDescription/articleDescription.html.twig', [
    //         'article' => $article,
    //         'commentaires' => $article->getCommentaires(),
    //         'form' => $form->createView(),
...
//         // Vérification du contenu du commentaire via le service IA
//         if (!$this->openAIService->isCommentAcceptable($commentContent)) {
//             // Le commentaire est jugé inapproprié : on le sauvegarde avec l'état "non valide"
//             $commentaire->setEtat("non valide");
//             $this->addFlash('warning', 'Votre commentaire a été sauvegardé, mais il est marqué comme non valide.');

//         } else {
//             // Le commentaire est acceptable : on le marque comme valide (ou on laisse l'état par défaut)
//             $commentaire->setEtat("valide");
//         }

//         // On affecte l'article et l'utilisateur au commentaire
//         $commentaire->setArticle($article);
//         $commentaire->setUser($this->getUser());

//         // On persiste et enregistre le commentaire dans la base de données
//         $this->em->persist($commentaire);
//         $this->em->flush();
